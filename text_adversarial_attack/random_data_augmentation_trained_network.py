...
import network
import dataset
import random_data_augmentation_dataset
import helpers
...
def get_network(dataset_type: dataset.DatasetType, encoding_type: dataset.Encoding, number_augmented=-1, kernel_variation=[3, 4, 5], ignore_cached=False, cache_prefix=''):
    """
    ...
    """
    # Try loading cached
    if not ignore_cached and os.path.exists('./models/{}-{}-{}trained-with-random-data-augmentation-{}-{}.h5'.format(dataset_type.value, encoding_type.value, cache_prefix, str(kernel_variation), number_augmented)):
        return keras.models.load_model('./models/{}-{}-{}trained-with-random-data-augmentation-{}-{}.h5'.format(dataset_type.value, encoding_type.value, cache_prefix, str(kernel_variation), number_augmented))

    # Create dataset - always create a new dataset to reduce the effect of randomness
    (x_train, y_train), (x_test, y_test), class_labels = random_data_augmentation_dataset.get_standard_dataset(dataset_type, encoding_type, number_augmented=number_augmented)
    ...
    metadata = dict()
    metadata['trainings_dataset'] = {'dataset': dataset_type.value, 'text encoding': encoding_type.value, 'size': len(x_train), 'normal_number': len(x_train)-number_augmented, 'number_augmented': number_augmented, 'augmented_percentage': number_augmented/len(x_train)}
    ...
    with open('./models/{}-{}-{}trained-with-random-data-augmentation-{}-{}.metadata'.format(dataset_type.value, encoding_type.value, cache_prefix, str(kernel_variation), number_augmented), 'w') as file:
        json.dump(metadata, file, indent=4,)

    model.save('./models/{}-{}-{}trained-with-random-data-augmentation-{}-{}.h5'.format(dataset_type.value, encoding_type.value, cache_prefix, str(kernel_variation), number_augmented))
...
if __name__ == '__main__':
    for dataset_type in dataset.DatasetType:
        for number_augmented in [0, 500, 1000, 1500, 2000]:
            print('Dataset: {}, Number augmented data: {}'.format(dataset_type.value, number_augmented))
            model = get_network(dataset_type, dataset.Encoding.WORD2VEC, number_augmented=number_augmented)
